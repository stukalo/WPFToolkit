...
namespace Standard
{
	internal sealed class MessageWindow : DispatcherObject, IDisposable
	{
		...
		private static IntPtr _WndProc(IntPtr hwnd, WM msg, IntPtr wParam, IntPtr lParam)
		{
			...
			if (msg == WM.CREATE)
			{
				GCHandle gCHandle = GCHandle.FromIntPtr(((CREATESTRUCT)Marshal.PtrToStructure(lParam, typeof(CREATESTRUCT))).lpCreateParams);
				target = (MessageWindow)gCHandle.Target;
			...
			zero = (wndProc == null ? Standard.NativeMethods.DefWindowProc(hwnd, msg, wParam, lParam) : wndProc(hwnd, msg, wParam, lParam));
			if (msg == WM.NCDESTROY)
			{
